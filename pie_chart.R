...
pie_chart <- function() {
  ...
  age_group_amt.data <- data.frame(Age_Group = c("7+", "13+", "16+", "18+", 
                                                 "all"),
                                   Amount = c(num_seven, num_thirteen, 
                                              num_sixteen, num_eighteen, num_all),
                                   prop = c(round(num_seven/total * 100, 2), 
                                            round(num_thirteen/total* 100, 2), 
                                            round(num_sixteen/total * 100, 2), 
                                            round(num_eighteen/total * 100, 2), 
                                            round(num_all/total * 100, 2)))
  
  #add label position
  
  mycols <- c("#CD5334", "#073B4C", "#88527F", "#BA1200", "#379392")
  
  age_group_amt.data$Age_Group <- factor(age_group_amt.data$Age_Group, levels=c("7+", "13+", "16+", 
                                                            "18+", "all"), 
  ...
  #create pie chart
  pie_chartt <- ggplot(age_group_amt.data, aes(x = "", y = prop, 
                                               fill = Age_Group)) +
