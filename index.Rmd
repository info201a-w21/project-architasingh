...
<<<<<<< HEAD
  In the summary information function, we first made a list to store basic information like the number of observations/features. There were `r summary_info$num_observations` observations, `r summary_info$num_features` features, and the names of the features were `r summary_info$feature_names`. We then determined the highest ratings for both IMDb and Rotten Tomatoes and counted the number of ratings with the highest value. The highest rating on IMDb had a value of `r summary_info$IMDb_highest_rating` and there were `r summary_info$num_IMDb_highest_rating` ratings with this value. The highest rating on Rotten Tomatoes was `r summary_info$RT_highest_rating` and there were `r summary_info$num_RT_highest_rating` ratings with this value.
=======
  In the summary information function, we first made a list to store basic information like the number of observations/features. The number of observations was `r summary_info$num_observations` and the number of features was `r summary_info$num_features` and the names of the features were `r summary_info$feature_names`. We then determined the highest ratings for both IMDb and Rotten Tomatoes and counted the number of ratings with the highest value. The highest rating on IMDb had a value of `r summary_info$IMDb_highest_rating[[1]]` and the number of ratings with this value was `r summary_info$num_IMDb_highest_rating`. The highest rating on Rotten Tomatoes was `r summary_info$RT_highest_rating[[1]]` and the number of ratings with this value was `r summary_info$num_RT_highest_rating`.
>>>>>>> 8d5a2ea478bf7339bc7e45100b8dec88dab07227
